...
class ImageComponent extends Component {
	...
	function square_cut($src_file , $name , $length){

		$image = new GdImage($src_file);
		$image->name = $name;
		$new_filepath = $image->fullpath();
		$w = $image->image_width;
		...
		if($w > $h){
			//1回高さでリサイズ
			$image->height = $length;
			$image->width = round($w * $length/$h);
			$image->save();
			//その後余分な両端をトリム
			$image = new GdImage($new_filepath);
			$image->crop = true;
			$image->left = round(($w * $length/$h - $length)/2);
			$image->top = 0;
		...
		//縦長
			...
			$image->width = $length;
			$image->height = round($h * $length/$w);
			$image->left = 0;
			$image->top = round(($h * $length/$w - $length)/2);
			$image->width = $image->height = $length;
			$image->save();
		//正方形
		}else{
...
class GdImage{
    ...
    function fullpath(){
		return $this->dir.DIRECTORY_SEPARATOR.$this->name.'.'.$this->ext;
    }
    ...
    function save($show=false) {
        ...
        imagecopyresampled($new_image, $image, 0, 0, $this->left, $this->top, $this->width, $this->height, $this->image_width, $this->image_height);

 ...
 * 誕生日メールを送信する。
 *
 * 【送り方】誕生日当日のペットを抽出→c_reminder_mailに格納→c_reminder_mailsを全てメール送信
 */
class BirthdayMailShell extends Shell {
	...
	function main() {
		...
		$cond = "Pet.birthday LIKE '%" . $birthday . "' AND Pet.birthday_type=1";
		...
		$cond = "ReminderMail.send_estim_date='" . $today . "' AND ReminderMail.send_flg=0 AND table_id=11 ";
		...
		foreach($datas as $data) {
			...
			$pet_param['birthday'] = $pet_karte['birthday'];
			$pet_param['today'] = $birthday;
			
			$now = date('Ymd');
    		$birthday_buf = str_replace("-", "", $pet_param['birthday']);
		...
		$cond = "ReminderMail.send_estim_date='" . $today . "' AND ReminderMail.table_id=11";
		$setdata = array();
	...
	function mailSend($mailaddress, $name, $subject, $pet_param) {
	    ...
	    */
	    ...
	    $email
	                ->template('birthday_mail', 'birthday_mail')
	                ->viewVars($ary_vars)
	                ->emailFormat('text')

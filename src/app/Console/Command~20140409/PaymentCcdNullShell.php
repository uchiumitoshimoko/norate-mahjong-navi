 ...
 * 当月分の支払書データを作成する。
 */
class PaymentCcdNullShell extends Shell {
		...
		$con_date = (!empty($this->args[0])) ? substr($this->args[0],0,4).'-'.substr($this->args[0],4,2):date('Y-m');
                ...
                $option = array(
                        ...
                        'joins' => array(
                                ...
                                'conditions' => 'CcdPaymentDetail.ccd = Contractor.ccd'),
                        ),
                        'group' => 'CcdPaymentDetail.ccd',
                        'fields' => 'Contractor.*',
                );

		$Contractors = $CcdPaymentDetail->find('all',$option);
...
*/
			...
			$save_data['CcdPayment']['pay_yyyymm'] = (!empty($this->args[0])) ? substr($this->args[0],0,4).substr($this->args[0],4,2):date('Ym');
			...
			// $sheet->setCellValue('B20', $save_data['CcdPayment']['bank']); //金融機関名
			// $sheet->setCellValue('B21', $save_data['CcdPayment']['siten']); // 支店
			// $sheet->setCellValue('B22', $save_data['CcdPayment']['koza']); // 口座
			// $sheet->setCellValue('B23', $save_data['CcdPayment']['meigi']); // 名義
			// $sheet->setCellValue('G26', $save_data['CcdPayment']['kurikosi_price']); // 前月繰越金額
			...
			$Ftt =array();
			$opt1 = array(
				'fields' => array ('CcdPaymentDetail.*','count(1) as count','SUM(pay_price) as sum_price'),
				'conditions' => array (
//					'CcdPaymentDetail.type' => '1' ,
					'CcdPaymentDetail.pay_month' => $con_date ,
//					'CcdPaymentDetail.pay_price >' => '0' , 
					'OR' => array('CcdPaymentDetail.ccd' => $ccd,'CcdPaymentDetail.pay_matome_ccd' => $ccd)
				),
...
/*
			$log = $CcdPaymentDetail->getDataSource()->getLog(false, false);
			$querylog = $log['log'][$log['count']-1]['query'];
var_dump($querylog);exit;
*/
			$opt1['fields'] = array('CcdPaymentDetail.*');
			unset($opt1['group']);

			$Customers_option = $CcdPaymentDetail->find('all',$opt1);
...
*/			
			...
			if (is_array($Customers)){
				...
				$sql = "SELECT cname FROM m_contractors WHERE ccd='" . $Customer['CcdPaymentDetail']['ccd'] . "'";
				$m_contractors_data = $CcdPaymentDetail->query($sql, false);
				...
				$row['pay_price']=$Customer['CcdPaymentDetail']['pay_price'];
				$rate=$row['rate']/100;
				$row['count']=$Customer[0]['count'];
				$row['price']=$Customer['CcdPaymentDetail']['price'];
				// $row['products_prise']=$Customer['CcdPaymentDetail']['price'] * $Customer[0]['count'];
				if ( $Customer['CcdPaymentDetail']['type'] == 3 ){
				    if($Customer[0]['sum_price'] * 1.0 >=0 ) {
					    $row['products_prise']=floor($Customer[0]['sum_price'] * 1.05);
					}
					else {
						$row['products_prise']=ceil($Customer[0]['sum_price'] * 1.05);
					}
				} else {
				    $row['products_prise']=str_replace("","",floor(($Customer['CcdPaymentDetail']['price']*$Customer[0]['count']) - ($Customer[0]['sum_price']*1.05)));
				}
				...
				$sheet2->setCellValue('A'.$i2,$Customer_option['CcdPaymentDetail']['application_date']); // 申込日
				$sheet2->setCellValue('B'.$i2,$Customer_option['CcdPaymentDetail']['customer_cd']); // ID
				$sheet2->setCellValue('C'.$i2,$Customer_option['CcdPaymentDetail']['name_kana']); // 氏名
				$sheet2->setCellValue('D'.$i2,str_replace("-","",$Customer_option['CcdPaymentDetail']['price'])); // 単価
				$sheet2->setCellValue('E'.$i2,$Customer_option['CcdPaymentDetail']['rate']); // 手数料
				//$sheet2->setCellValue('F'.$i2,str_replace("-","",$Customer_option['CcdPaymentDetail']['pay_price'])); // 手数料単価
				$sheet2->setCellValue('F'.$i2,$Customer_option['CcdPaymentDetail']['pay_price']); // 手数料単価
				//$res_tax += $row['price'] * $rate - $row['price'] / 1.05 * $rate;
				
				$res_tax += $row['price'] * $rate - floor(ceil($row['price'] / (($Customer_option['CcdPaymentDetail']['tax_rate']+100)/100)) * $rate);
				
				//$res_tax += $row['price'] * $rate - floor((ceil($row['price'] / (($Customer_option['CcdPaymentDetail']['tax_rate']+100)/100)) * $rate);
				
				$Customer_option['CcdPaymentDetail']['payment_id'] = $insert_id;

				$CcdPaymentDetail->save($Customer_option);
...
/*
			...
			if (is_array($KCustomers)){
				...
				$row['product_name']=$Cont['Contractor']['cname']."　継続会員";
				$row['rate']=$KCustomer['CcdPaymentDetail']['rate'];
...
*/			
			...
			$save_data['CcdPayment']['total_price'] = $total_prise; //合計支払金額
			...
			switch ($Cont['Contractor']['pay_site']){
				case "0":
					$save_data['CcdPayment']['pay_date'] = date('Y/m/d', mktime(0, 0, 0, intval(substr($this->args[0],4,2))+1, 0, intval(substr($this->args[0],0,4))));
				...
				break;
				case "1":
					$save_data['CcdPayment']['pay_date'] = date('Y/m/d', mktime(0, 0, 0, intval(substr($this->args[0],4,2))+2, 0, intval(substr($this->args[0],0,4))));
					// Invoice No. CHE-130901-01
				...
				break;
				case "2":
					$save_data['CcdPayment']['pay_date'] = date('Y/m/d', mktime(0, 0, 0, intval(substr($this->args[0],4,2))+3, 0, intval(substr($this->args[0],0,4))));
					// Invoice No. CHE-130901-01
			...
			// $sheet->setCellValue('G25', $save_data['CcdPayment']['monthly_price']); //今月分支払金額
			
			// 1万円未満は、支払日を印字しない。
			if(intval($save_data['CcdPayment']['monthly_price']) >= 10000) {
				$payweek = date('w',strtotime($save_data['CcdPayment']['pay_date']));
				if ( $payweek == 0 ) {
				    $pay_date = date('Y/m/d',mktime(0,0,0,substr($save_data['CcdPayment']['pay_date'],5,2),substr($save_data['CcdPayment']['pay_date'],8,2)-2,substr($save_data['CcdPayment']['pay_date'],0,4)));
				} elseif ( $payweek == 6 ) {
				    $pay_date = date('Y/m/d',mktime(0,0,0,substr($save_data['CcdPayment']['pay_date'],5,2),substr($save_data['CcdPayment']['pay_date'],8,2)-1,substr($save_data['CcdPayment']['pay_date'],0,4)));
				} else {
				    $pay_date = $save_data['CcdPayment']['pay_date'];
				}
			...
			$sheet->setCellValue('B17', $res_prise); // 合計支払金額
			$sheet->setCellValue('F42', $res_prise); // 今月分支払金額

                    ...
                    <!-- /Sidebar -->
                                <script>
                                 $(function(){
                                    ...
                                    $('.select-tab .tab-job-info').click(function(){
                                        $('.select-tab > li').removeClass('active');
                                        $(this).addClass('active');
                                        
                                        $('.select-info > div').hide();
                                        $('.select-info .job-info').show();
                                    });
                                    $('.select-tab .tab-corporate-info').on('click',function(){
                                        $('.select-tab > li').removeClass('active');
                                        $(this).addClass('active');
                                        $('.select-info > div').hide();
                                        $('.select-info .corporate-info').show();
                                    });
                                });                                     
                                </script>                    
...
<?
	$company_name = "";
	if($jobs['Jobs']['company_name'] != "社名：非公開企業") {
//		$company_name = "会社名：" . $jobs['Jobs']['company_name'];
		$company_name = "" . $jobs['Jobs']['company_name'];
	}
	else {
		$company_name = "非公開企業";
	}
	...
	if($image_list[1] != "") {
?>
                            <div class="image-main" style="background:url(/search/read_company_name/<?=h($image_list[1]);?>);">
                                <h3 class="title"><?=$company_name?>
<?
	// 実績がある場合にメダルを表示
	if($jobs['Jobs']['jisseki_flg'] == "1") {
?>
                                	<div class="medal"><img src="/img/medal.png" alt="紹介実績あり"></div>
<?
	}
?>
                                </h3>
...
?>
                                <h3 class="logo-image"><img src="/top_banners/read_image/<?=h($top_banners['TopBanners']['id']);?>"></h3>
<?
		}
?>
                            </div>
...
?>
                        ...
                        <div class="detail__item">
                            ...
                            <div class="item-main">
                                ...
                                <div class="title"><?=$jobs['Jobs']['title']?></div>
                                <div class="catchcopy"><?=$company_name?></div>
								...
								<div class="catchcopy">職種コード：<?=$syokusyu_name?></div>

<?
	// 実績がある場合にメダルを表示
	if($jobs['Jobs']['jisseki_flg'] == "1") {
?>
                                <div class="icon-perform">紹介実績あり</div>
<?
	}
?>
...
?>
                            <div class="image-one" >
                               <img src="/search/read_company_name/<?=h($image_list[2]);?>">
                            </div>
<?
	}
?>
...
?>
                                ...
                                <img src="/search/read_company_name/<?=h($image_list[3]);?>">
<?
	}
?>
...
?>
                                ...
                                <img src="/search/read_company_name/<?=h($image_list[4]);?>">
<?
	}
?>
                            </div>
                            ...
                            <ul class="select-tab">    
                                <li class="tab-job-info active">募集要項</li>
                                <li class="tab-corporate-info ">会社情報</li>
                            </ul>  
...
?>
									...
									<?
										if(!empty($jobs['Jobs']['company_tokutyo'])) {
									?>
                                    <tr class="pcs">
                                        <th>コンサルタントからの一言</th>
                                        <td>
                                        	...
                                        	<?
                                        		if($no_image_flg == "1") {
											?>
		                                        	<img src="/search/read_no_image/<?=$jobs['Jobs']['id']?>">
		                                    <?
		                                    	}
		                                    ?>
                                    ...
                                    <tr class="smps">
                                        <td colspan="2" class="smps-td" id="smps-td004">
                                            <h3 class="mobile-th">コンサルタントからの一言</h3>
                                        </td>
                                    </tr>
	                                <?
                                    	}
								      ...
								      <section class="inner ">
								        ...
								        <div class="guidelines">
								         <dl class="item_category">
								          ...
								          <h1 class="title01" style="background:#2F3E92;color:#FFF;padding:5px 10px;margin-bottom:1rem;">求人</h1>
								            
								            <div class="inner">
								            <dt></dt>
								            
								            	<?
								            		foreach($jobs_company as $row) {
												?>
								             <dd><a href="/search/detail/<?=$row['Jobs']['id']?>"><?=$row['Jobs']['title']?></a></dd>
												<?
													}
												?>
								             </div>
                                ...
                                <div class="" style=" display: block;
                                        ...
                                        <a href="/entry/?jobid=<?=$jobs['Jobs']['id']?>">
                                            <img src="/img/02_entry_sp.jpg" style="max-width:100%; float:none; margin:0; display:inline;">
                                        </a>
...
?>
            ...
            <!-- testimonials -->
            <div class="toppage__testimonials">
                <div class="container">
                    <h2 class="title">この求人と似た職種で転職された方の声</h2>

                    <ul class="toppage__testimonials-list">
                    
                  	<?
                  		$age = Configure::read('age');
						
                  		foreach($voices as $voice) {
							$sex = "";
							$sex_name = "";
							if($voice['Voice']['sex'] == "1") {
								$sex = "is--male";
								$sex_name = "男性";
							}
							else if($voice['Voice']['sex'] == "2") {
								$sex = "is--female";
								$sex_name = "女性";
							}
					?>
                        <!-- item -->
                        <li class="toppage__testimonials-item <?=$sex?>">
                            <a style="color:black" href="/voices/detail/<?=$voice['Voice']['id']?>">
                                <p class="toppage__testimonials-comment">
                                    <?=$voice['Voice']['list_comment']?>
                                </p>


                                <div class="speaker">
                                    <div class="post"><?=$voice['Voice']['syokusyu_name']?></div>
                                    <div class="status"><?=$age[$voice['Voice']['age']]?> / <?=$sex_name?></div>
                                </div>
					...
					<!--
                    <div class="new-button"><a href="#">すべての新着をみる</a></div>
					-->
                </div>
...
window.criteo_q.push(  
        ...
        { event: "viewItem", item: "<?=$jobs['Jobs']['job_id']?>" }
);
</script>
...
<script type="application/ld+json"> {
    ...
    "@type" : "JobPosting",
    "title" : "<?=str_replace('"', '', $jobs['Jobs']['title'])?>",
    "description" : "<p><strong>仕事内容</strong></p>
    <p><?=str_replace('"', '', $jobs['Jobs']['kinmu_naiyo'])?></p>
    <p><strong>求める経験</strong></p>
    <p><?=str_replace('"', '', $jobs['Jobs']['gyomukeiken'])?></p>",
    "datePosted" : "<?=date('Y-m-d', strtotime($jobs['Jobs']['update_date']))?>",
    ...
    "baseSalary": {
        ...
        "value": {
            ...
            "minValue": <?=$nensyu_min?>0000,
            "maxValue": <?=$nensyu_max?>0000,
            "unitText": "MONTH"
        }
    }
}
